...
def print_progressbar(iteration, total, prefix="", suffix="", decimals=1, length=100, fill='█'):  # pragma: no cover
    ...
    if "execute_query" in tic_toc:
        ...
        print(terminal.move_y(3) + f"{prefix} |{terminal.color_rgb(r, g, b)}{bar}{terminal.normal}| {percent}% "
                                   f"{suffix} {round(duration, 2)}s")
        print(f"time left: {round(((duration / float(percent)) * 100) - duration, 2)}s                                ")
        if len(values) > 1:
            pass
            print(terminal.move_y(6) + f"rate   : {round((1 / numpy.mean(values)) * config.concurrency, 3)}qs/s       ")
